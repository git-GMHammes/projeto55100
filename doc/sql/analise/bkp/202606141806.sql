...
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`%` SQL SECURITY DEFINER VIEW `view_partidos_2022_RJ` AS
select distinct `votacao_candidato_munzona_2022_RJ`.`SG_PARTIDO` AS `SG_PARTIDO`,
    `votacao_candidato_munzona_2022_RJ`.`NM_PARTIDO` AS `NM_PARTIDO`
from `votacao_candidato_munzona_2022_RJ`
where (
        `votacao_candidato_munzona_2022_RJ`.`SG_PARTIDO` is not null
    )
order by `votacao_candidato_munzona_2022_RJ`.`SG_PARTIDO`;
DROP TABLE IF EXISTS `view_partidos_2024_RJ`;
CREATE ALGORITHM = UNDEFINED DEFINER = `root` @`%` SQL SECURITY DEFINER VIEW `view_partidos_2024_RJ` AS
select distinct `votacao_secao_2024_RJ`.`SG_PARTIDO` AS `SG_PARTIDO`,
    `votacao_secao_2024_RJ`.`NM_PARTIDO` AS `NM_PARTIDO`
from `votacao_secao_2024_RJ`
where (`votacao_secao_2024_RJ`.`SG_PARTIDO` is not null)
order by `votacao_secao_2024_RJ`.`SG_PARTIDO`;
